...
//从配置文件中得到所有的服务器IP地址，然后构建出可用的IP地址列表
ngx_int_t
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    us->peer.init = ngx_http_upstream_init_round_robin_peer;//默认使用round robin
    ...
    if (us->servers) {
        ...
        for (i = 0; i < us->servers->nelts; i++) {			//遍历服务器数组
            if (server[i].backup) {							//计算非backup服务器
                continue;
            }
        ...
        peers->name = &us->host;							//upstream块的名字

        n = 0;

        for (i = 0; i < us->servers->nelts; i++) {			//遍历服务器数组
            for (j = 0; j < server[i].naddrs; j++) {		//遍历服务器的每个IP地址
                if (server[i].backup) {						//只处理非backup服务器
                    continue;
                }

                peers->peer[n].sockaddr = server[i].addrs[j].sockaddr;//socket地址
                peers->peer[n].socklen = server[i].addrs[j].socklen;  //socket结构长度
                peers->peer[n].name = server[i].addrs[j].name;		  //地址的名字
        ...
        peers->next = backup;					//挂载backup服务器列表
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    rrp->peers = us->peer.data;//得到配置里的IP地址列表
    rrp->current = 0;
	...
	//设置 get/free 回调函数，供upstream框架在连接服务器时调用，否则还是默认的round robin算法
