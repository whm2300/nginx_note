...
//该模块是ngx_modules模块的最后一个元素，同时也是过滤链表的第一个元素，它处理响应头，检查if_modified_since头，决定是否返回304状态码
...
//检查请求头，如果文件没有被修改，就设置HTTP状态码为NGX_HTTP_NOT_MODIFIED，然后调用ngx_http_next_header_filter执行后续的过滤模块
...
ngx_http_not_modified_filter_init(ngx_conf_t *cf)
{
    ...
    ngx_http_top_header_filter = ngx_http_not_modified_header_filter;//ngx_http_not_modified_header_filter为模块的处理函数
