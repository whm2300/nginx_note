...
static ngx_command_t  ngx_http_upstream_ip_hash_commands[] = {
    ...
    { ngx_string("ip_hash"),
      ...
      ngx_http_upstream_ip_hash,				//指令解析函数
      0,
      0,
      NULL },
...
{
    if (ngx_http_upstream_init_round_robin(cf, us) != NGX_OK) {	//调用round robin函数初始化IP地址表
        return NGX_ERROR;
    }

    us->peer.init = ngx_http_upstream_init_ip_hash_peer;		//设置自己的算法初始化函数
...
//该函数为IP散列算法的具体实现，根据地址计算散列值，然后从列表中选择一个地址
static ngx_int_t
ngx_http_upstream_get_ip_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (iphp->tries > 20 || iphp->rrp.peers->single) {           //重试次数过多
        return iphp->get_rr_peer(pc, &iphp->rrp);				//退化为round robin
    }
...
ngx_http_upstream_ip_hash(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    uscf->peer.init_upstream = ngx_http_upstream_init_ip_hash;		//设置算法入口
