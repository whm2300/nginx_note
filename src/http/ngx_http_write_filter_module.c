...
//用于输出响应体，两个参数ngx_request_t和ngx_chain_t，第一个就是请求，第二个参数是输出内容的buffer链表
ngx_int_t
ngx_http_write_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    *ll = NULL;
    ...
    if (r->limit_rate) {
		...
		/* 超出发送速率限制 */
        if (limit <= 0) {
            c->write->delayed = 1; /* 设置delayed标记，延迟发送请求 */  
            ngx_add_timer(c->write,
                          (ngx_msec_t) (- limit * 1000 / r->limit_rate + 1));

            c->buffered |= NGX_HTTP_WRITE_BUFFERED; /* 设置buffered标记 */  
...
{
    ngx_http_top_body_filter = ngx_http_write_filter; // 初始化链表头结点
