    ...
    ngx_log_t *log)
{
    ...
    n = sendmsg(s, &msg, 0); //发送消息,sendmsg为linux的系统调用，也是通过socket发送消息
...
ngx_read_channel(ngx_socket_t s, ngx_channel_t *ch, size_t size, ngx_log_t *log)
{
    ...
    n = recvmsg(s, &msg, 0); //recvmsg为linux系统调用，接受消息
    ...
    if ((size_t) n < sizeof(ngx_channel_t)) { //接受到的消息丢失
    ...
    if (ch->command == NGX_CMD_OPEN_CHANNEL) {  //按照指令处理接受到的信息
