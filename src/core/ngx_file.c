    ...
    ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
    ...
    cln = ngx_pool_cleanup_add(pool, sizeof(ngx_pool_cleanup_file_t)); //向回收链申请了sizeof(ngx_pool_cleanup_file_t)的空间，存放临时文件的信息
    if (cln == NULL) {
        return NGX_ERROR;
    }
	//下面的代码将实际的临时文件信息与回收链刚申请的空间关联在一起
    for ( ;; ) {
        ...
        if (file->fd != NGX_INVALID_FILE) {
			...
			cln->handler = clean ? ngx_pool_delete_file : ngx_pool_cleanup_file;// 指向临时文件的处理程序 delete_file或cleanup_file
            clnf = cln->data; // 将申请的内存空间赋值给clnf,用于存放信息
