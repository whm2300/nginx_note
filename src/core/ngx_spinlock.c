...
/* 自旋锁 注释来自：http://blog.csdn.net/walkerkalr/article/details/38237147?utm_source=tuicool&utm_medium=referral
...
ngx_spinlock(ngx_atomic_t *lock, ngx_atomic_int_t value, ngx_uint_t spin)
{
	...
	// 一直处于循环中，直到获取到锁
    for ( ;; ) {
		// lock为0表示没有其他进程持有锁，这时将lock值设置为value参数表示当前进程持有了锁 ,ngx_atomic_cmp_set为原子操作
